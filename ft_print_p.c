...
/*   Updated: 2023/11/24 12:02:29 by sforster         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_print_l(long long int n, int base)
{
	int		count;
	char	*basesymbols;
	...
	{
		count += ft_print_l(n / (long long int)base, base);
		n = n % base;
	}
	if (n < (long long int)base)
	{
...
int	ft_print_p(void *n)
{
	...
	if (n == NULL)
		return (count += write(1, "0", 1));
	count += ft_print_l((long long int)n, 16);
